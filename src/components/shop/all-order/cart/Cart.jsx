...
const Cart = () => {
    ...
    let grandTotal = deliveryCost === 40 ? (totalMrp + gst + deliveryCost ): (totalMrp + gst);
...
//  HEAD
            ...
            cartProducts.length === 0 ?  <div className="emp-con"><img  className="empty-img"
            src="/images/empty-cart.svg" alt="Cart Empty"/>
            <span className="empty-cart">Empty Cart</span></div> :
//  origin/v2vt
                <>
                    ...
                    <Grid item xs={4} className="amout-details">
                        <Paper className="cart-price">
                            ...
                            <p className="amount-heading">Billing Details</p><hr className="underline"/>
                                <p>No. of distinct products <span className="data" >{totalNoOfItem}</span></p>
                                <p>Total Mrp <span className="data" >{totalMrp.toFixed(2)}</span></p>
                                <p>Delivery Cost<span className="data" >{deliveryCost === 40 ? deliveryCost.toFixed(2) : deliveryCost}</span></p>
                                <p>GST <span className="data" >{gst.toFixed(2)}</span></p>
                                <p>Total Amount<span className="data" >{grandTotal.toFixed(2)}</span></p>
                            </div>
                            <Link to="/shop/order">
